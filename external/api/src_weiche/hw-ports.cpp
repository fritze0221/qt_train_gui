...
static HWP_PORT port_tab[ HWP_SIZE ];
...
void hwp_init()
	...
	{
	   port_tab[ i ].address = addr_list[ i ];
	   port_tab[ i ].mode    = HWP_UNUSED;
	}
...
int hwp_read( int nr )
    {
       ...
       else p->last_data = digitalRead( p->address );
       return p->last_data;
    }
